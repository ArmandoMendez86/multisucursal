...
class AperturaCaja {
    ...
    /**
     * Registra la apertura de caja para una sucursal y fecha específicas.
     ...
     * @throws Exception Si ya existe una apertura para esa fecha y sucursal o hay un error en la base de datos.
     */
    public function registrarApertura($id_usuario, $id_sucursal, $fecha_apertura, $monto_inicial) {
        // Primero, verificar si ya existe una apertura para esta fecha y sucursal
        if ($this->obtenerAperturaPorFecha($id_sucursal, $fecha_apertura)) {
            throw new Exception("La caja ya ha sido abierta para esta sucursal en la fecha {$fecha_apertura}.");
        }
    ...
    /**
     * Obtiene el registro de apertura de caja para una sucursal y fecha específicas.
     * @param int $id_sucursal ID de la sucursal.
     ...
     */
    public function obtenerAperturaPorFecha($id_sucursal, $fecha_apertura) {
        $query = "SELECT id, id_usuario, id_sucursal, fecha_apertura, monto_inicial, created_at
                  FROM aperturas_caja
                  WHERE id_sucursal = :id_sucursal AND fecha_apertura = :fecha_apertura";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':id_sucursal', $id_sucursal, PDO::PARAM_INT);
        $stmt->bindParam(':fecha_apertura', $fecha_apertura);
        $stmt->execute();
