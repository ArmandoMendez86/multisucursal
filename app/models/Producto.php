...
class Producto
{
    ...
    public function create($data, $id_sucursal_actual)
    {
        ...
        try {
            ...
            if (!empty($data['codigos_barras']) && is_array($data['codigos_barras'])) {
                ...
                foreach ($data['codigos_barras'] as $codigo) {
                    ...
                    if (!empty($codigo_limpio)) {
                        $stmt_codigos->execute([':id_producto' => $id_producto_nuevo, ':codigo_barras' => $codigo_limpio]);
                    }
                }
            }
            ...
            if ($stock_inicial_actual > 0) {
                $this->addInventoryMovement($id_producto_nuevo, $id_sucursal_actual, $_SESSION['user_id'], 'entrada', $stock_inicial_actual, 0, $stock_inicial_actual, 'Inventario inicial por creación de producto', null);
            }
            $this->conn->commit();
    ...
    public function update($id, $data, $id_sucursal)
    {
        ...
        try {
            ...
            $stmt_delete = $this->conn->prepare("DELETE FROM " . $this->codes_table . " WHERE id_producto = :id_producto");
            $stmt_delete->execute([':id_producto' => $id]);
            if (!empty($data['codigos_barras']) && is_array($data['codigos_barras'])) {
                ...
                foreach ($data['codigos_barras'] as $codigo) {
                    $codigo_limpio = trim($codigo);
                    if (!empty($codigo_limpio)) {
                        $stmt_insert->execute([':id_producto' => $id, ':codigo_barras' => $codigo_limpio]);
                    }
                }
            }

            $query_inventario = "INSERT INTO " . $this->inventory_table . " (id_producto, id_sucursal, stock, stock_minimo) VALUES (:id_producto, :id_sucursal, :stock, :stock_minimo) ON DUPLICATE KEY UPDATE stock = :stock_update, stock_minimo = :stock_minimo_update";
            ...
            $stmt_inventario->execute([
                ...
                ':stock_minimo_update' => $data['stock_minimo']
            ]);

            if ($oldStock != $data['stock']) {
                $this->addInventoryMovement($id, $id_sucursal, $_SESSION['user_id'], 'ajuste', abs($data['stock'] - $oldStock), $oldStock, $data['stock'], 'Actualización de producto desde gestión', null);
            }
    ...
    {
        $query = "SELECT p.*, inv.stock, inv.stock_minimo, GROUP_CONCAT(pc.codigo_barras) as codigos_barras FROM " . $this->table_name . " p LEFT JOIN " . $this->inventory_table . " inv ON p.id = inv.id_producto AND inv.id_sucursal = :id_sucursal LEFT JOIN " . $this->codes_table . " pc ON p.id = pc.id_producto WHERE p.id = :id GROUP BY p.id";
        $stmt = $this->conn->prepare($query);
    ...
    {
        $baseQuery = "FROM productos p LEFT JOIN categorias c ON p.id_categoria = c.id LEFT JOIN marcas m ON p.id_marca = m.id LEFT JOIN inventario_sucursal inv ON p.id = inv.id_producto AND inv.id_sucursal = :id_sucursal LEFT JOIN producto_codigos pc ON p.id = pc.id_producto";
    ...
    {
        $query = "SELECT p.id AS producto_id, p.sku, p.nombre AS producto_nombre, s.nombre AS sucursal_nombre, inv.stock FROM {$this->table_name} p JOIN {$this->inventory_table} inv ON p.id = inv.id_producto JOIN sucursales s ON inv.id_sucursal = s.id LEFT JOIN {$this->codes_table} pc ON p.id = pc.id_producto WHERE (p.sku LIKE :searchTerm OR p.nombre LIKE :searchTerm OR pc.codigo_barras LIKE :searchTerm) AND p.activo = 1 GROUP BY p.id, s.id ORDER BY p.nombre, s.nombre";
        $stmt = $this->conn->prepare($query);
        $likeTerm = "%{$searchTerm}%";
